...
// from https://github.com/xx-labs/web-wallet/blob/fcdcd4f5ddeeb69c272e4550fde3353ecd0328b7/packages/custom-derives/src/staking/stakerRewards.ts
...
function parseRewards(
  ...
  return exposures.map(
    ({
      ...
      const { eraPoints, validators: allValPoints } = erasPoints.find((p) =>
        p.era.eq(era)
      ) || { eraPoints: BN_ZERO, validators: {} as DeriveEraValPoints };
      const { eraReward } = erasRewards.find((r) => r.era.eq(era)) || {
        eraReward: api.registry.createType("Balance"),
      };
      const { validators: allValPrefs } = erasPrefs.find((p) =>
        p.era.eq(era)
      ) || { validators: {} as DeriveEraValPrefs };
      ...
      Object.entries(eraValidators).forEach(([validatorId, exposure]): void => {
        ...
        const valComm =
          allValPrefs[validatorId]?.commission.unwrap() || BN_ZERO;
        const expTotal =
          exposure.total?.unwrap().add(exposure.custody?.unwrap()) || BN_ZERO;
        let avail = BN_ZERO;
        ...
        if (!(expTotal.isZero() || valPoints.isZero() || eraPoints.isZero())) {
          ...
          if (validatorId === stakerId) {
            ...
            const stakerExp = exposure.others.find(({ who }) =>
              who.eq(stakerId)
            );
...
function allUniqValidators(
  rewards: DeriveStakerReward[][]
): [string[], string[][]] {
  return rewards.reduce(
    ([all, perStash]: [string[], string[][]], rewards) => {
      const uniq: string[] = [];
...
function removeClaimed(
  validators: string[],
  queryValidators: DeriveStakingQuery[],
  reward: DeriveStakerReward
): void {
  const rm: string[] = [];
  ...
  Object.keys(reward.validators).forEach((validatorId): void => {
    ...
    if (index !== -1) {
      ...
      if (
        validatorId !== valLedger.stash.toString() ||
        valLedger?.claimedRewards.some((e) => reward.era.eq(e))
      ) {
        rm.push(validatorId);
      }
    }
...
function filterRewards(
  ...
  const filter = eras.filter(
    (e) => !stakingLedger.claimedRewards.some((s) => s.eq(e))
  );
    ...
    .map((reward) =>
      objectSpread({}, reward, {
        nominators: reward.nominating.filter(
          (n) => reward.validators[n.validatorId]
        ),
      })
    );
}

export function _stakerRewardsEras(
  instanceId: string,
  api: DeriveApi
): (eras: EraIndex[], withActive?: boolean) => Observable<ErasResult> {
  return memo(
    instanceId,
    (eras: EraIndex[], withActive = false): Observable<ErasResult> =>
      combineLatest([
  ...
  return memo(
    ...
    (
      ...
      combineLatest([
        ...
        switchMap(
          ([queries, exposures, erasResult]): Observable<
            DeriveStakerReward[][]
          > => {
            const allRewards = queries.map(
              ({ stakingLedger, stashId }, index): DeriveStakerReward[] =>
            ...
            const [allValidators, stashValidators] =
              allUniqValidators(allRewards);

            return api.derive.staking
              .queryMulti(allValidators, { withLedger: true })
              .pipe(
                map((queriedVals): DeriveStakerReward[][] =>
                  queries.map(
                    ({ stakingLedger }, index): DeriveStakerReward[] =>
                      filterRewards(
                        eras,
                        stashValidators[index].map(
                          (validatorId): [string, DeriveStakingQuery] => [
                            validatorId,
                            queriedVals.find((q) =>
                              q.accountId.eq(validatorId)
                            ) as DeriveStakingQuery,
                          ]
                        ),
                        {
...
export const stakerRewards = firstMemo(
  (api: DeriveApi, accountId: Uint8Array | string, withActive?: boolean) =>
    api.derive.staking
      ...
      .pipe(
        switchMap((eras) =>
          api.derive.staking._stakerRewards([accountId], eras, withActive)
        )
      )
);

export function stakerRewardsMultiEras(
  instanceId: string,
  api: DeriveApi
): (
  accountIds: (Uint8Array | string)[],
...
export function stakerRewardsMulti(
  instanceId: string,
  api: DeriveApi
): (
  accountIds: (Uint8Array | string)[],
  ...
  return memo(
    ...
    (
      ...
      api.derive.staking
        ...
        .pipe(
          switchMap((eras) =>
            api.derive.staking.stakerRewardsMultiEras(accountIds, eras)
          )
        )
  );
}
