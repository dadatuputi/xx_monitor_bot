...
export async function execute(ctx: XXContext, db: Database) {
  ...
  const inlineKeyboard = new InlineKeyboard()
    .text(`${Icons.ADD} Add Node`, "add")

  // First display a list of monitored nodes, if any
  await ctx.reply(
    await buildResponseText(db, user.id.toString()),
      { 
...
export const convos = {
  add: async (conversation: XXConversation, ctx: XXContext, db: Database) => {
    let cmix_id: string;
    do {
      cmix_id = await conversation.form.text( (ctx) => { ctx.reply("Please provide a valid cmix ID.")});
      await ctx.reply("Please provide a valid cmix ID\\.\n_Valid IDs are 44 characters long and base64 encoded\\._",
      {
        parse_mode: "MarkdownV2"
      })
    ...
    const cmix_node_name = await conversation.form.text( (ctx) => { ctx.reply("Please provide a friendly name for the node.")})

    const status = await db.addNode(ctx.from!.id.toString(), cmix_id, cmix_node_name); // returns false if the user is already monitoring this node/name combination
    ...
    if (status !== undefined) {
      ...
      if ("modifiedCount" in status) {
        ...
        const monitoring = `${Icons.WATCH}  Monitoring ${prettify_address_alias(cmix_node_name, cmix_id)}. Reporting changes here.`;
      }
    } else {
